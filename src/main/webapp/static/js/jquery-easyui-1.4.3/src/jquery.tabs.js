...
(function($){
	...
	function addTools(container){
		...
		var header = $(container).children('div.tabs-header');
		if (opts.mapTools) {
			if (typeof opts.mapTools == 'string'){
				$(opts.mapTools).addClass('tabs-tool').appendTo(header);
				$(opts.mapTools).show();
			} else {
				header.children('div.tabs-tool').remove();
				var tools = $('<div class="tabs-tool"><table cellspacing="0" cellpadding="0" style="height:100%"><tr></tr></table></div>').appendTo(header);
				var tr = mapTools.find('tr');
				for(var i=0; i<opts.mapTools.length; i++){
					var td = $('<td></td>').appendTo(tr);
					var tool = $('<a href="javascript:void(0);"></a>').appendTo(td);
					tool[0].onclick = eval(opts.mapTools[i].handler || function(){});
					tool.linkbutton($.extend({}, opts.mapTools[i], {
						plain: true
					}));
				}
			}
		...
		if (param.type == 'all' || param.type == 'header'){
			...
			if (opts.header){
				...
				if (opts.mapTools){
					...
					if ($.isArray(opts.mapTools)){
						p_tool.empty();
						for(var i=0; i<opts.mapTools.length; i++){
							var t = $('<a href="javascript:void(0)"></a>').appendTo(p_tool);
							t.addClass(opts.mapTools[i].iconCls);
							if (opts.mapTools[i].handler){
								t.bind('click', {handler:opts.mapTools[i].handler}, function(e){
						...
						$(opts.mapTools).children().appendTo(p_tool);
					}
