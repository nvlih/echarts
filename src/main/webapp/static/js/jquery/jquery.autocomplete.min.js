...
(function($) {
	...
	$.Autocompleter = function(input, options) {
		...
		$input.bind(
			...
			if(e.keyCode== KEY.PAGEUP||e.keyCode==KEY.PAGEDOWN||e.keyCode== KEY.UP||e.keyCode== KEY.DOWN){
				return;
			}
			$input.trigger("search",function(){
				$input.trigger("show");
 			});
 		});
		function selectCurrent() {
			...
			$input.val(v);
			hideResultsNow();
			console.info("trigger",selected.data, selected.value);
		...
		function request(term, success, failure) {
			...
			if (data && data.length) {
				...
				$.ajax({
					...
					url : options.url,
					jsonp:options.jsonp||"dataCallback",
					data : $.extend({
